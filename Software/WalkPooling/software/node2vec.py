...
def CalN2V(edge_index,args,num_nodes):
    modeljure = Node2Vec(edge_index, embedding_dim=args.embedding_dim, walk_length=20,
                     context_size=10, walks_per_node=10,
                     num_negative_samples=1, p=1, q=1, sparse=True, num_nodes=num_nodes).to(device)
    ...
    for i in range(args.epoch_num):
        ...
        if i%20 == 0:
            print(f'Setp: {i:03d} /{args.epoch_num}, Loss : {total_loss:.4f}')
